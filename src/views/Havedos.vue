<template>
  <div>
    <Card
      ...
      @done="done(index)" />
    <div v-if="havedoLen <= 0" class="font-weight-thin display-1 text-xs-center grey--text">No Havedos.</div>
  </div>
</template>
...
export default class Havedos extends Vue {

  private get todos() { return this.$store.state.todos; }

  private get havedoLen() {
    return this.todos.filter(({ type }: { type: TODOTYPE }) => (type === TODOTYPE.HAVEDO)).length;
  }

  private get commit() { return this.$store.commit; }

  private type: string = TODOTYPE.HAVEDO;
